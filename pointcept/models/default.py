...
@MODELS.register_module()
class DefaultSegmentorGlobal(nn.Module):
    def __init__(
        self,
        num_classes,
        backbone_out_channels,
    ...
    def forward(self, input_dict, return_point=False):
        ...
        # train
        if self.training:
            loss = self.criteria(seg_logits[input_dict["com_mask"] == 0], input_dict["segment_all"][input_dict["com_mask"] == 0])
            return_dict["loss"] = loss
        # eval
        elif "segment_all" in input_dict.keys():
            loss = self.criteria(seg_logits[input_dict["com_mask"] == 0], input_dict["segment_all"][input_dict["com_mask"] == 0])
            return_dict["loss"] = loss
            return_dict["seg_logits"] = seg_logits
        # test
            ...
            return_dict["seg_logits"] = seg_logits
            return_dict["com_mask"] = input_dict["com_mask"]
        return return_dict


@MODELS.register_module()
class KDSegmentorGlobalTeacher(nn.Module):
    def __init__(
        self,
        num_classes,
        backbone_out_channels,
    ...
    def forward(self, input_dict):
        point = Point(input_dict)
        return_dict = self.backbone(point)
        seg_logits = self.seg_head(return_dict["dec_feat_unmask"])
        return_dict["seg_logits"] = seg_logits[input_dict["com_mask"] == 0]
        return return_dict


@MODELS.register_module()
class KDSegmentorGlobalStudent(nn.Module):
    def __init__(
        self,
        num_classes,
        backbone_out_channels,
    ...
    def forward(self, input_dict):
        point = Point(input_dict)
        return_dict = self.backbone(point)
        seg_logits = self.seg_head(return_dict["dec_feat"])
        # train
        if self.training:
            loss = self.criteria(seg_logits, input_dict["segment_all"][input_dict["com_mask"] == 0])
            return_dict["loss"] = loss
            return_dict["seg_logits"] = seg_logits
        # eval
        elif "segment_all" in input_dict.keys():
