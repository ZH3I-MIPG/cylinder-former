...
def xy2key(
    ...
    if depth > 8:
        ...
        x_all = x_h | x_l
        y_all = y_h | y_l
        key = x_all << (depth - 8) | y_all
    else:
        key = (EX[x & mask] >> (8 - depth)) | EY[y & mask]
...
def encode_3d_spiral_hilbert(locs: torch.Tensor, m_xy: int, m_z: int):
    z = locs[..., 2]
    s = xy2key(locs[..., 0], locs[..., 1], depth=m_xy // 2)
    locs_2d = torch.stack([z, s], dim=-1)
    return encode(locs_2d, num_dims=2, num_bits=m_z)
