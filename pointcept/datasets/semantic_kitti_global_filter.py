...
class SemanticKITTIGlobalFilterDataset(DefaultDataset):
    ...
    def get_test_frame(self, data_path):
        dir_path, file_name = os.path.split(data_path)
        frame_num = file_name.split('.')[0]
        dir_path = "/home/ubuntu/JK/VoxelTransformer/third_party/PointTransformerV3/Pointcept/exp/semantic_kitti/exp3_globalv25_720_test/result"
        new_file_name = "08_" + frame_num + "_pred.npy"
    ...
    def get_complementary_frame_old(self, data_path):
        ...
        # 计算目标帧的编号
        target_frame_prev = f"{int(frame_num) - 3:06d}.bin"
        target_frame_next = f"{int(frame_num) + 3:06d}.bin"
    ...
    def get_data(self, idx):
        ...
        com_data_path = self.get_complementary_frame_old(data_path)
        ...
        # 定义保留的标签集合 # np.array([1, 2, 4, 5, 6, 7, 9, 11, 13, 15, 16, 17, 18])
        valid_labels = np.array([1, 2, 4, 5, 6, 7, 9, 11, 13, 15, 16, 17, 18]) #np.array([1, 2, 4, 5, 7, 9, 10, 11, 13, 14, 15, 16, 17, 18])# 0 3 6 8 10 12

        # 创建掩码，只保留标签在 valid_labels 中的点
        filter_mask = np.in1d(com_segment, valid_labels)

        # 应用掩码，过滤掉无效的点
        com_coord = com_coord[filter_mask]
        com_strength = com_strength[filter_mask]
        com_segment = com_segment[filter_mask]
