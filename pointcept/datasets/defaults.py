...
class DefaultDataset(Dataset):
    ...
    def get_data(self, idx):
        ...
        if "coord_all" in data_dict.keys():
            data_dict["coord_all"] = data_dict["coord_all"].astype(np.float32)
        ...
        if "segment_all" in data_dict.keys():
            data_dict["segment_all"] = data_dict["segment_all"].reshape([-1]).astype(np.int32)
        else:
            data_dict["segment_all"] = (
                np.ones(data_dict["coord_all"].shape[0], dtype=np.int32) * -1
            )
    ...
    def prepare_test_data(self, idx):
        ...
        if "segment" in data_dict.keys():
            ...
            # result_dict = dict(segment=data_dict.pop("segment"))
        else:
            result_dict = dict(segment=data_dict.pop("segment_all"), mask_all=data_dict.pop("mask_all"), name=data_dict.pop("name"))
