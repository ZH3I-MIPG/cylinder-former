...
def index_operator_global(data_dict, index, duplicate=False):
    if "index_valid_keys" not in data_dict:
        data_dict["index_valid_keys"] = [
            "coord_all",
            "strength_all",
...
class PointClip(object):
    ...
    def __call__(self, data_dict):
        ...
        if "com_coord" in data_dict.keys():
            data_dict["com_coord"] = np.clip(
                data_dict["com_coord"],
                a_min=self.point_cloud_range[:3],
                a_max=self.point_cloud_range[3:],
            )
        if "coord_all" in data_dict.keys():
            data_dict["coord_all"] = np.clip(
                data_dict["coord_all"],
                a_min=self.point_cloud_range[:3],
                a_max=self.point_cloud_range[3:],
            )
...
class RandomRotate(object):
    ...
    def __call__(self, data_dict):
        ...
        if "com_coord" in data_dict.keys():
            if self.center is None:
                x_min, y_min, z_min = data_dict["com_coord"].min(axis=0)
                x_max, y_max, z_max = data_dict["com_coord"].max(axis=0)
                center = [(x_min + x_max) / 2, (y_min + y_max) / 2, (z_min + z_max) / 2]
            else:
                center = self.center
            data_dict["com_coord"] -= center
            data_dict["com_coord"] = np.dot(data_dict["com_coord"], np.transpose(rot_t))
            data_dict["com_coord"] += center
        if "coord_all" in data_dict.keys():
            if self.center is None:
                x_min, y_min, z_min = data_dict["coord_all"].min(axis=0)
                x_max, y_max, z_max = data_dict["coord_all"].max(axis=0)
                center = [(x_min + x_max) / 2, (y_min + y_max) / 2, (z_min + z_max) / 2]
            else:
                center = self.center
            data_dict["coord_all"] -= center
            data_dict["coord_all"] = np.dot(data_dict["coord_all"], np.transpose(rot_t))
            data_dict["coord_all"] += center
...
class RandomScale(object):
    ...
    def __call__(self, data_dict):
        ...
        if "com_coord" in data_dict.keys():
            ...
            data_dict["com_coord"] *= scale
        if "coord_all" in data_dict.keys():
            scale = np.random.uniform(
                self.scale[0], self.scale[1], 3 if self.anisotropic else 1
            )
            data_dict["coord_all"] *= scale
        return data_dict
...
class RandomFlip(object):
    ...
    def __call__(self, data_dict):
        if np.random.rand() < self.p:
            ...
            if "com_coord" in data_dict.keys():
                data_dict["com_coord"][:, 0] = -data_dict["com_coord"][:, 0]
            if "coord_all" in data_dict.keys():
                data_dict["coord_all"][:, 0] = -data_dict["coord_all"][:, 0]
            if "normal" in data_dict.keys():
        ...
        if np.random.rand() < self.p:
            ...
            if "com_coord" in data_dict.keys():
                data_dict["com_coord"][:, 1] = -data_dict["com_coord"][:, 1]
            if "coord_all" in data_dict.keys():
                data_dict["coord_all"][:, 1] = -data_dict["coord_all"][:, 1]
            if "normal" in data_dict.keys():
...
class RandomJitter(object):
    ...
    def __call__(self, data_dict):
        ...
        if "com_coord" in data_dict.keys():
            jitter = np.clip(
                self.sigma * np.random.randn(data_dict["com_coord"].shape[0], 3),
                -self.clip,
                self.clip,
            )
            data_dict["com_coord"] += jitter
        if "coord_all" in data_dict.keys():
            jitter = np.clip(
                self.sigma * np.random.randn(data_dict["coord_all"].shape[0], 3),
                -self.clip,
                self.clip,
            )
            data_dict["coord_all"] += jitter
        return data_dict
...
def cart2polar(input_xyz):
    ...
    phi_deg = np.degrees(phi) / 10
    return np.stack((rho, phi_deg, input_xyz[:, 2]), axis=1)

def polar2cat(input_xyz_polar):
    phi = np.radians(input_xyz_polar[:, 1] * 10)
    x = input_xyz_polar[:, 0] * np.cos(phi)
    y = input_xyz_polar[:, 0] * np.sin(phi)
    return np.stack((x, y, input_xyz_polar[:, 2]), axis=1)


@TRANSFORMS.register_module()
class PolarGlobalSample(object):
    def __init__(
        self,
        grid_size=0.05,
        sample_shape=(960, 720, 64),
        point_cloud_range=(0, -18, -4, 35.2, 18, 2),
        hash_type="fnv",
        mode="train",
    ):
        self.grid_size=grid_size
        self.sample_shape = np.asarray(sample_shape)
        self.point_cloud_range = point_cloud_range
        self.hash = self.fnv_hash_vec if hash_type == "fnv" else self.ravel_hash_vec
        assert mode in ["train", "test"]
        self.mode = mode
    ...
    def __call__(self, data_dict):
        assert {"coord_all", "com_mask"}.issubset(data_dict.keys())

        coord_all = data_dict["coord_all"]
        com_mask = data_dict["com_mask"]
        polar_all = cart2polar(coord_all)

        if self.grid_size is not None:
            scaled_coord = polar_all / np.array(self.grid_size)
            grid_all = np.floor(scaled_coord).astype(int)
        else:
            max_bound_r = np.percentile(polar_all[:, 0], 100, axis=0)
            min_bound_r = np.percentile(polar_all[:, 0], 0, axis=0)
            max_bound = np.max(polar_all[:, 1:], axis=0)
            min_bound = np.min(polar_all[:, 1:], axis=0)
            max_bound = np.concatenate(([max_bound_r], max_bound))
            min_bound = np.concatenate(([min_bound_r], min_bound))
            crop_range = max_bound - min_bound
            cur_grid_size = self.sample_shape
            intervals = crop_range / (cur_grid_size - 1)

            if (intervals == 0).any(): print("Zero interval!")
            grid_all = (np.floor((np.clip(polar_all, min_bound, max_bound) - min_bound) / intervals)).astype(int)

        min_coord = grid_all.min(0)
        grid_all -= min_coord
        key = self.hash(grid_all)
        idx_sort = np.argsort(key)
        key_sort = key[idx_sort]
        _, inverse, count = np.unique(key_sort, return_inverse=True, return_counts=True)
        polar_all = np.clip(
        ...
        data_dict["coord_all"] = polar2cat(polar_all)
        ...
        if self.mode == "train":
            ...
            idx_unique = idx_sort[idx_select]
            data_dict = index_operator_global(data_dict, idx_unique)
            data_dict["com_mask"] = com_mask[idx_unique]  # 来源标识
            data_dict["grid_all"] = grid_all[idx_unique]
            return data_dict
            ...
            for i in range(count.max()):
                idx_select = np.cumsum(np.insert(count, 0, 0)[0:-1]) + i % count
                idx_part = idx_sort[idx_select]
                part = index_operator_global(data_dict.copy(), idx_part, duplicate=True)
                part["index"] = idx_part
                part["grid_all"] = grid_all[idx_part]
                data_part_list.append(part)
            return data_part_list
        else:
            raise NotImplementedError
...
class SphereGlobalCrop(object):
    ...
    def __call__(self, data_dict):
        point_max = (
            int(self.sample_rate * data_dict["coord_all"].shape[0])
            if self.sample_rate is not None
            else self.point_max
        )

        assert "coord_all" in data_dict.keys()
        if data_dict["coord_all"].shape[0] > point_max:
            if self.mode == "random":
                center = data_dict["coord_all"][
                    np.random.randint(data_dict["coord_all"].shape[0])
                ]
            elif self.mode == "center":
                center = data_dict["coord_all"][data_dict["coord_all"].shape[0] // 2]
            else:
                raise NotImplementedError
            idx_crop = np.argsort(np.sum(np.square(data_dict["coord_all"] - center), 1))[
                :point_max
            ]
            data_dict = index_operator(data_dict, idx_crop)
        return data_dict
