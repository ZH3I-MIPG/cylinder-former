...
@TESTERS.register_module()
class SemSegGlobalTester(TesterBase):
    def test(self):
        assert self.test_loader.batch_size == 1
        logger = get_root_logger()
        logger.info(">>>>>>>>>>>>>>>> Start Evaluation >>>>>>>>>>>>>>>>")
        ...
        for idx, data_dict in enumerate(self.test_loader):
            ...
            if "segment" in data_dict.keys():
                segment_all = data_dict.pop("segment")
            else:
                segment_all = data_dict.pop("segment_all")
            mask_all = data_dict.pop("mask_all")
            data_name = data_dict.pop("name")
            # segment = segment_all[data_dict["com_mask"] == 0]
            pred_save_path = os.path.join(save_path, "{}_pred.npy".format(data_name))
            if os.path.isfile(pred_save_path):
                logger.info(
                    "{}/{}: {}, loaded pred and label.".format(
                ...
                pred = np.load(pred_save_path)
                mask_all_cuda = torch.from_numpy(mask_all).cuda()
                segment_all_cuda = torch.from_numpy(segment_all).cuda()
                segment = segment_all_cuda[mask_all_cuda == 0]
                ...
                segment = segment.data.cpu().numpy()
                pred = torch.zeros((segment.size, self.cfg.data.num_classes)).cuda()
                pred_all = torch.zeros((segment_all.size, self.cfg.data.num_classes)).cuda()
                # com_mask = torch.zeros(segment_all.size).cuda()
                for i in range(len(fragment_list)):
                    fragment_batch_size = 1
                    s_i, e_i = i * fragment_batch_size, min(
                        (i + 1) * fragment_batch_size, len(fragment_list)
                    ...
                    idx_part = input_dict["index"]
                    with torch.no_grad():
                        pred_part_all = self.model(input_dict)
                        pred_part = pred_part_all["seg_logits"]  # (n, k)
                        # pred_mask = pred_part_all["com_mask"]
                        # pred_part = pred_part[pred_mask == 0]
                        pred_part = F.softmax(pred_part, -1)
                        if self.cfg.empty_cache:
                            torch.cuda.empty_cache()
                        bs = 0
                        for be in input_dict["offset"]:
                            pred_all[idx_part[bs:be], :] += pred_part[bs:be]
                            # com_mask[idx_part[bs:be]] += pred_mask[bs:be]
                            bs = be
                        pred = pred_all[mask_all_cuda == 0]
                        # segment_all_cuda = torch.from_numpy(segment_all).cuda()  # or .long() / .bool() / …
                        # segment = segment_all_cuda[com_mask == 0]
...
@TESTERS.register_module()
class KDSemSegGlobalTester(TesterBase):
    def test(self):
        assert self.test_loader.batch_size == 1
        logger = get_root_logger()
        logger.info(">>>>>>>>>>>>>>>> Start Evaluation >>>>>>>>>>>>>>>>")
        ...
        for idx, data_dict in enumerate(self.test_loader):
            ...
            # segment = segment_all[data_dict["com_mask"] == 0]
            pred_save_path = os.path.join(save_path, "{}_pred.npy".format(data_name))
            if os.path.isfile(pred_save_path) and False:
                logger.info(
                    "{}/{}: {}, loaded pred and label.".format(
                        idx + 1, len(self.test_loader), data_name
                    )
                ...
                for i in range(len(fragment_list)):
                    ...
                    for key in input_dict.keys():
                        if isinstance(input_dict[key], torch.Tensor):
                            input_dict[key] = input_dict[key].cuda(non_blocking=True)
                    idx_part = input_dict["index"][input_dict["com_mask"] == 0]
                    with torch.no_grad():
                        pred_part_all = self.model(input_dict)
                        pred_part = pred_part_all["seg_logits"]  # (n, k)
                        # pred_mask = pred_part_all["com_mask"]
