    ...
    def eval(self):
        ...
        for i, input_dict in enumerate(self.trainer.val_loader):
            ...
            if "segment" in input_dict:
                ...
                segment = input_dict["segment_all"]
...
@HOOKS.register_module()
class SemSegEvaluatorPro(HookBase):
    def __init__(self, write_cls_iou=False):
        self.write_cls_iou = write_cls_iou
    ...
    @torch.no_grad()
    def batch_confusion_matrix(self, pred, target, num_classes, ignore_index=None):
        """
        pred, target: 1-D torch.Tensor, 相同长度
        return: [num_classes, num_classes]  cpu tensor
        """
    ...
    def eval(self):
        ...
        for i, input_dict in enumerate(self.trainer.val_loader):
            ...
            # 原有交集/并集统计
            intersection, union, target = intersection_and_union_gpu(
                pred, segment, num_classes, self.trainer.cfg.data.ignore_index
            )
            if comm.get_world_size() > 1:
            ...
            # <ADD> 逐 batch 更新混淆矩阵
            cm_batch = self.batch_confusion_matrix(
                pred, segment, num_classes, self.trainer.cfg.data.ignore_index
            )
            cm_total += cm_batch
        ...
        # <ADD> 跨卡 reduce 混淆矩阵
        if comm.get_world_size() > 1:
            dist.all_reduce(cm_total)
        cm_total = cm_total.cpu().numpy()
        ...
        # <ADD> 从混淆矩阵计算 Precision / Recall / F1
        tp = np.diag(cm_total)
        ...
        for i in range(num_classes):
            self.trainer.logger.info(
                "Class_{idx}-{name} Result: "
                "iou/accuracy/precision/recall/f1 "
                "{iou:.4f}/{accuracy:.4f}/{precision:.4f}/{recall:.4f}/{f1:.4f}".format(
                    idx=i,
                    name=self.trainer.cfg.data.names[i],
                    iou=iou_class[i],
                    accuracy=acc_class[i],
        ...
        # 打印带表头的混淆矩阵
        # ------------------------------------------------------------------
        names = getattr(self.trainer.cfg.data, "names", None)
        if names is None or len(names) != num_classes:
            names = [f"cls_{i}" for i in range(num_classes)]

        # 1. 列宽：按最长类别名 + 数值宽度
        col_width = max(len(n) for n in names) + 2
        col_width = max(col_width, 7)  # 至少 7 位，容纳 “0.1234” 或 “1234567”

        # 2. 构造表头
        header = "GT\\Pred".ljust(col_width) + "".join(n.ljust(col_width) for n in names)
        separator = "-" * len(header)

        lines = [header, separator]
        for gt_idx, row in enumerate(cm_total):
            line = names[gt_idx].ljust(col_width) + "".join(
                f"{val:>{col_width - 1}d}" for val in row)  # 整数，右对齐
            lines.append(line)
        ...
        if self.trainer.writer is not None:
            ...
            for i in range(num_classes):
                self.trainer.writer.add_scalar(
                    f"val/cls_{i}-{self.trainer.cfg.data.names[i]} IoU",
                    iou_class[i], current_epoch
                )
                # <ADD> 也可以把 precision/recall/f1 写进 TB
                self.trainer.writer.add_scalar(
                    f"val/cls_{i}-{self.trainer.cfg.data.names[i]} Precision",
                    precision[i], current_epoch
                )
                self.trainer.writer.add_scalar(
                    f"val/cls_{i}-{self.trainer.cfg.data.names[i]} Recall",
                    recall[i], current_epoch
                )
                self.trainer.writer.add_scalar(
...
@HOOKS.register_module()
class KDSemSegEvaluator(HookBase):
    def __init__(self, write_cls_iou=False):
        self.write_cls_iou = write_cls_iou
        ...
        for i, input_dict in enumerate(self.trainer.val_loader):
            for key in input_dict.keys():
                if isinstance(input_dict[key], torch.Tensor):
                    input_dict[key] = input_dict[key].cuda(non_blocking=True)
            with torch.no_grad():
                output_dict = self.trainer.s_model(input_dict)
            output = output_dict["seg_logits"]
            loss = output_dict["loss"]
            pred = output.max(1)[1]
            segment = input_dict["segment_all"][input_dict["com_mask"] == 0]
...
@HOOKS.register_module()
class GlobalSemSegEvaluator(HookBase):
    def __init__(self, write_cls_iou=False):
        self.write_cls_iou = write_cls_iou
