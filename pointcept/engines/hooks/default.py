...
@HOOKS.register_module()
class KDModelHook(HookBase):
    def before_train(self):
        if comm.get_world_size() > 1 and isinstance(
            self.trainer.s_model.module, HookBase
        ):
            self.model = weakref.proxy(self.trainer.s_model.module)
        elif isinstance(self.trainer.s_model, HookBase):
            self.model = weakref.proxy(self.trainer.s_model)
        else:
            self.model = HookBase()
        self.model.trainer = self.trainer
        self.model.before_train()
